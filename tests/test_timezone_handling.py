...
class TestHeartbeatTimezoneHandling:
    ...
    def test_heartbeat_timeout_utc_vs_local(self, postgres):
        ...
        old_heartbeat = utc_now.subtract(seconds=10)
    ...
    def test_heartbeat_timeout_different_timezones(self, postgres):
        ...
        for tz in timezones:
            tz_time = current_time.in_timezone(tz)
            with postgres.connect() as conn:
                conn.execute(text(f"""
                    INSERT INTO {tables["Node"]} (name, created_on, last_heartbeat)
                    VALUES (:name, :created_on, :heartbeat)
    ...
    def test_heartbeat_comparison_after_dst_change(self, postgres):
        ...
        dst_transition = pendulum.parse('2024-03-10 01:00:00', tz='America/New_York')
        before_dst = dst_transition.subtract(hours=2)
...
class TestLockExpirationTimezones:
    ...
    def test_lock_expiration_utc_vs_local(self, postgres):
        ...
        expired_utc = utc_now.subtract(days=2)
        expired_ny = ny_now.subtract(days=2)
        valid_utc = utc_now.add(days=1)
    ...
    def test_lock_expiration_boundary_conditions(self, postgres):
        ...
        utc_midnight = pendulum.now('UTC').start_of('day')
        ny_midnight = pendulum.now('America/New_York').start_of('day')
        tokyo_midnight = pendulum.now('Asia/Tokyo').start_of('day')

        # Lock expires at UTC midnight (in the past)
        expires_at_utc_midnight = utc_midnight.subtract(days=1)
        ...
        with postgres.connect() as conn:
            ...
            """), {
                ...
                'created_at': utc_midnight.subtract(days=2),
                'created_by': 'test',
...
class TestLeaderLockTimezones:
    ...
    def test_stale_leader_lock_detection_different_timezones(self, postgres):
        ...
        stale_time = ny_time.subtract(seconds=400)
...
class TestLeaderElectionTimezones:
    ...
    def test_leader_election_with_mixed_timezones(self, postgres):
        ...
        base_utc = pendulum.now('UTC')

        nodes = [
            ('node-tokyo', base_utc.in_timezone('Asia/Tokyo').subtract(seconds=30)),
            ('node-london', base_utc.in_timezone('Europe/London').subtract(seconds=20)),
            ('node-ny', base_utc.in_timezone('America/New_York').subtract(seconds=10)),
        ]
    ...
    def test_leader_election_timestamp_comparison_across_dst(self, postgres):
        ...
        before_dst = pendulum.parse('2024-03-09 23:00:00', tz='America/New_York')
        after_dst = pendulum.parse('2024-03-10 03:00:00', tz='America/New_York')
    ...
    def test_token_assigned_at_different_timezones(self, postgres):
        ...
        with postgres.connect() as conn:
            ...
            for i, tz in enumerate(timezones):
                tz_time = base_time.in_timezone(tz)
                conn.execute(text(f"""
                    INSERT INTO {tables["Node"]} (name, created_on, last_heartbeat)
                    VALUES (:name, :created_on, :heartbeat)
                """), {
...
class TestRebalanceTimingTimezones:
    ...
    def test_membership_change_detection_across_timezones(self, postgres):
        ...
        old_heartbeat = utc_time.subtract(seconds=20)
        with postgres.connect() as conn:
            conn.execute(text(f"""
                UPDATE {tables["Node"]}
...
class TestDatabaseTimezoneConsistency:
    ...
    def test_database_timezone_vs_client_timezone(self, postgres):
        ...
        client_time = pendulum.now('America/Chicago')
    ...
    def test_timestamp_without_timezone_compatibility(self, postgres):
        ...
        # PostgreSQL stores "timestamp without time zone" but we use timezone-aware pendulum
        ...
        try:
            ...
            created_on = pendulum.instance(node[0])
            last_heartbeat = pendulum.instance(node[1])
    ...
    def test_audit_timestamps_use_database_timezone(self, postgres):
        ...
        try:
            ...
            client_time_utc = pendulum.now('UTC')
            client_time_tokyo = pendulum.now('Asia/Tokyo')
            client_time_db = pendulum.now(db_timezone)
            ...
            for record in audit_records:
                stored_time = pendulum.instance(record['created_on'])
                logger.info(f"Task {record['item']}: stored as {stored_time}")

                age_seconds = abs((pendulum.now('UTC') - stored_time).total_seconds())
                assert_true(age_seconds < 30,
                           f'Audit timestamp should be recent (age: {age_seconds}s)')

            utc_stored = pendulum.instance(audit_records[0]['created_on'])
            tokyo_stored = pendulum.instance(audit_records[1]['created_on'])
...
class TestDatetimeParameterTimezones:
    ...
    def test_datetime_with_different_timezone(self, postgres):
        ...
        tokyo_dt = pendulum.now('Asia/Tokyo').replace(year=2024, month=3, day=15, hour=14)
        tokyo_native = tokyo_dt.in_timezone('Asia/Tokyo')
    ...
    def test_datetime_with_local_timezone(self, postgres):
        ...
        local_dt = pendulum.now().replace(year=2024, month=3, day=15, hour=14)
    ...
    def test_datetime_timezone_preserved_in_audit(self, postgres):
        ...
        test_cases = [
            ...
            ('tokyo', pendulum.parse('2024-03-15 10:00:00', tz='Asia/Tokyo')),
            ('ny', pendulum.parse('2024-03-15 10:00:00', tz='America/New_York')),
        ]
    ...
    def test_datetime_comparison_across_timezones(self, postgres):
        ...
        same_moment_different_tz = [
            ...
            pendulum.parse('2024-03-15 05:00:00', tz='America/New_York'),
            pendulum.parse('2024-03-15 19:00:00', tz='Asia/Tokyo'),
        ]
    ...
    def test_datetime_date_boundary_timezones(self, postgres):
        ...
        late_tokyo = pendulum.parse('2024-03-15 23:59:59', tz='Asia/Tokyo')
        early_ny = pendulum.parse('2024-03-15 00:00:01', tz='America/New_York')
    ...
    def test_pendulum_datetime_types(self, postgres):
        """Verify Pendulum DateTime objects work correctly.
        """
        config = get_timezone_test_config()
        connection_string = postgres.url.render_as_string(hide_password=False)

        pendulum_dt = pendulum.parse('2024-03-15 14:30:00', tz='UTC')

        job = Job('test-pendulum', config, date=pendulum_dt, wait_on_enter=0,
                 connection_string=connection_string)
        ...
        job.__enter__()
        try:
            assert_true(job.am_i_healthy(), 'Job should be healthy with Pendulum datetime')
        finally:
            job.__exit__(None, None, None)


class TestDateConsistency:
    """Test that self._date is always a pendulum.Date object, never DateTime."""

    def test_date_none_returns_date_not_datetime(self, postgres):
        """Verify date=None results in pendulum.Date, not pendulum.DateTime.
        """
        config = get_timezone_test_config()
        connection_string = postgres.url.render_as_string(hide_password=False)
        ...
        assert_true(isinstance(job._date, pendulum.Date),
                   f'date=None should create pendulum.Date, got {type(job._date).__name__}')
        assert_true(not isinstance(job._date, pendulum.DateTime),
                   f'date=None should NOT create pendulum.DateTime, got {type(job._date).__name__}')

    def test_all_date_inputs_return_date_type(self, postgres):
        """Verify all date initialization paths result in pendulum.Date objects.
        """
        config = get_timezone_test_config()
        connection_string = postgres.url.render_as_string(hide_password=False)
        ...
        test_cases = [
            ...
            ('pendulum-datetime', pendulum.parse('2024-03-15 10:00:00', tz='UTC')),
        ]
        ...
        for label, date_input in test_cases:
            ...
            assert_true(isinstance(job._date, pendulum.Date),
                       f'{label}: should create pendulum.Date, got {type(job._date).__name__}')
            assert_true(not isinstance(job._date, pendulum.DateTime),
                       f'{label}: should NOT create pendulum.DateTime, got {type(job._date).__name__}')

            logger.info(f'✓ {label}: correctly creates pendulum.Date (type={type(job._date).__name__})')
    ...
    def test_date_type_prevents_timezone_issues(self, postgres):
        ...
        tokyo_dt = pendulum.parse('2024-03-15 23:00:00', tz='Asia/Tokyo')

        job = Job('test-date-only', config, date=tokyo_dt, wait_on_enter=0,
                 connection_string=connection_string)

        assert_true(isinstance(job._date, pendulum.Date), 'Should be Date type')
        assert_true(not isinstance(job._date, pendulum.DateTime), 'Should NOT be DateTime type')

        assert_equal(job._date.year, 2024, 'Year should match')
        assert_equal(job._date.month, 3, 'Month should match')
        assert_equal(job._date.day, 15, 'Day should preserve input date regardless of timezone')

        assert_true(not hasattr(job._date, 'timezone') or job._date.timezone is None,
                   'Date objects should not carry timezone info')
